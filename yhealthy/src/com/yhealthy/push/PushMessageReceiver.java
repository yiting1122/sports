...
public class PushMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, Intent intent) {
		...
		if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
			...
			responseIntent.setClass(context, SystemActivity.class);
...
//			context.startActivity(responseIntent);
			...
			if (errorCode == 0) {
				...
				util.put(context,"appid", appid);
				util.put(context,"channelId", channelid);
				util.put(context,"pushId", userid);

				if(util.getInt(context, "loginClass")==LoginConstants.SYSTEM_LOGIN)
				 {
				     MyPushService service=new MyPushService(context);
				    service.execute();
				 }
				else {
					MyPushService service=new MyPushService(context);
				    service.executeByOpen();
				}
			} else {
				if (errorCode == 30607) {
		...
		//可选。通知用户点击事件处理
			...
			aIntent.setClass(context, SystemActivity.class);
			String title = intent
